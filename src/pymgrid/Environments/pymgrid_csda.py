...
class MicroGridEnv(Environment):
    ...
    def __init__(self, env_config, seed=42):
        ...
        self.Na = (
            4 + self.mg.architecture["grid"] * 3 + self.mg.architecture["genset"] * 1
        )

        action_limits = [
            int(self.mg._pv_ts.max().values[0]),
        ...
        if self.mg.architecture["genset"] == 1:
            action_limits.append(
                int(
                    self.mg.parameters["genset_rated_power"].values[0]
                    * self.mg.parameters["genset_pmax"].values[0]
                )
            )
        ...
        self.action_space = gym.spaces.MultiDiscrete(action_limits)
        #     [gym.spaces.Discrete(x) for x in action_limits]
        # )
