...
class MicrogridEnv(Env, ABC):
    ...
    def __init__(self, microgrid, trajectory_len=None, max_episode_len=None):
        ...
        if isinstance(microgrid, int) and 0 <= microgrid <= 25:
            print(
                "Initializing microgrid {} of 25 using 25 microgrids from MicrogridGenerator".format(
                    microgrid
                )
            )
            ...
            raise ValueError(
                "microgrid must be of type Microgrid, is {}".format(type(microgrid))
            )

        assert (
            self.microgrid._data_length == 8760
        ), "Microgrid data length should be 8760, is {}".format(
            self.microgrid._data_length
        )
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=np.float("inf"), shape=(observation_dim,), dtype=np.float64
        )
    ...
    def _short_trajectory_set(self):
        ...
        if trajectory_len is not None:
            ...
            high_range = (
                self.microgrid._data_length - self.microgrid.horizon - trajectory_len
            )

            start_index = randint(low=0, high=high_range)
            self.microgrid._tracking_timestep = start_index
            self.microgrid._data_length = (
                start_index + trajectory_len + self.microgrid.horizon
            )
...
class ContinuousMicrogridEnv(MicrogridEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        super().__init__(
            microgrid, trajectory_len=trajectory_len, max_episode_len=max_episode_len
        )
        ...
        self.action_space = gym.spaces.Box(
            low=lower_bound, high=upper_bound, shape=(action_dim,), dtype=np.float64
        )
    ...
    def _get_action_ub_lb(self):
        ...
        if self.has_genset:
            p_genset_max = (
                ...
                * self.microgrid.parameters["genset_pmax"].values[0]
            )
    ...
    def get_values(self, *value_names):
        ...
        if self.current_action is None:
            print(
                "Warning: current_action is None, should only happen on first iteration"
            )
            if self.has_genset:
            ...
            action = self.standardize(
                self.current_action, action_mean, action_std, direction="backward"
            )
            obs = self.standardize(
                self.current_obs, obs_mean, obs_std, direction="backward"
            )
        else:
            action = self.current_action
            obs = self.current_obs
        ...
        for name in value_names:
            if name in actions_dict.keys():
                ...
                raise ValueError(
                    "Value '{}' not recognized with current architecture".format(name)
                )
    ...
    def reset(self):
        ...
        if self.standardization:
            obs_mean, obs_std, action_mean, action_std = self.standardizations
            observation = self.standardize(
                observation, obs_mean, obs_std, direction="forward"
            )
            self.current_obs = observation
    ...
    def step(self, action, **kwargs):
        ...
        if self.standardization:
            ...
            action = self.standardize(
                action, action_mean, action_std, direction="backward"
            )
    ...
    def standardize(
        self, data, mean_proxy=None, std_proxy=None, direction="forward", use_proxy=None
    ):
        """
        ...
        if (mean_proxy is None and std_proxy is None and use_proxy is None) or (
            mean_proxy is not None and use_proxy is not None
        ):
            raise ValueError(
                "Must pass mean_proxy and std_proxy, or use_proxy must be a str in ('action', 'obs'), but not both"
            )
        if mean_proxy is None and std_proxy is None:
            if use_proxy == "action":
                ...
                raise NameError(
                    "Unable to recognize use_proxy {}, must be one of 'action' or 'obs'".format(
                        use_proxy
                    )
                )
        ...
        for name, v in zip(names, vals):
            if not isinstance(v, np.ndarray):
                raise TypeError(
                    "{} must be of type numpy.ndarray, is {}".format(name, type(v))
                )
        if not (data.shape == mean_proxy.shape and mean_proxy.shape == std_proxy.shape):
            raise ValueError(
                "Incompatible shapes of data, mean_proxy, std_proxy. Must be equal, are: {}".format(
    ...
    def pre_compute_standardizations(self, alg_to_use="mpc"):
        ...
        if alg_to_use == "mpc":
            ...
            raise RuntimeError(
                "algorithm name {} not currently supported".format(alg_to_use)
            )
        ...
        for name, output in zip(names, outputs):
            if (output == 0).sum() != 0:
                for j, val in enumerate(output):
                    if val == 0:
                        print(
                            "Warning: Zero value in pos {} in {}, may not have been filled properly".format(
                                j, name
                            )
                        )
...
class ContinuousMicrogridSampleEnv(ContinuousMicrogridEnv):
    ...
    def __init__(
        ...
        super().__init__(
            microgrid, standardization=standardization, max_episode_len=max_episode_len
        )
    ...
    def reset(self, sampling_args=None):
        ...
        """
        sample_reset(
            self.has_grid, self.saa, self.microgrid, sampling_args=sampling_args
        )
...
class SafeExpMicrogridEnv(ContinuousMicrogridEnv):
    ...
    def _get_energy_balance(self):
        ...
        if self.scale_constraints:
            charge_scale_factor = float(
                self.microgrid.parameters.battery_capacity.squeeze()
            )
    ...
    def _get_inequality_constraints(self):
        ...
        p_charge, p_discharge, p_max_charge, p_max_discharge = self.get_values(
            "battery_charge", "battery_discharge", "capa_to_charge", "capa_to_discharge"
        )
        ...
        p_max_import = self.microgrid.parameters["grid_power_import"].values[0]
        p_max_export = self.microgrid.parameters["grid_power_export"].values[0]
        p_import, p_export, grid_status = self.get_values(
            "grid_import", "grid_export", "grid_status"
        )
        ...
        if self.has_genset:
            ...
            if p_genset < 1:
                if self.scale_constraints:
                    ...
                    constraints.append(
                        (-p_genset - self.balance_tolerance) / p_genset_max
                    )
                else:
