...
class Environment(gym.Env):
    ...
    def get_action_discrete_wrapper(self, action):
        """
        :param action: current action
        :return: control_dict : dicco of controls
        """
        ...
        if self.mg.architecture["battery"] == 1:
            if action[1] >= 0:
                control_dict["battery_charge"] = action[1] * self.mg.battery.capacity
            else:
                control_dict["battery_discharge"] = (
                    -action[1] * self.mg.battery.capacity
                )

        if self.mg.architecture["genset"] == 1:
            control_dict["genset"] = action[2] * self.mg.genset.rated_power_import

            if self.mg.architecture["grid"] == 1:
                if action[3] >= 0:
                    control_dict["grid_import"] = action[3] * self.mg.grid.power_import
                else:
                    control_dict["grid_export"] = -action[3] * self.mg.grid.power_export

        elif self.mg.architecture["grid"] == 1:
            if action[4] >= 0:
                control_dict["grid_import"] = action[4] * self.mg.grid.power_import
            else:
                control_dict["grid_export"] = -action[4] * self.mg.grid.power_export
