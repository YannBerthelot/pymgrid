...
class MicrogridGenerator:
    ...
    def generate_microgrid(self, verbose=True):
        ...
        for i in range(self.nb_microgrids):
            ...
            self.microgrids_regimes.append(
                self.split_microgrid_into_regimes(self.microgrids[i], starts, stops)
            )
        if verbose == True:
    ...
    def split_microgrid_into_regimes(self, mg, starts, stops):
        split_microgrid = []
        for i in range(len(starts)):
            mg_split = mg
            mg_split.pv = mg_split.pv[starts[i] : stops[i]]
            mg_split.load = mg_split.load[starts[i] : stops[i]]
            mg_split._next_load = mg_split._next_load[starts[i] : stops[i]]
            mg_split._next_pv = mg_split._next_pv[starts[i] : stops[i]]
            mg_split._grid_status_ts = mg_split._grid_status_ts[starts[i] : stops[i]]
            # self.grid_status = self._grid_status_ts.iloc[0, 0]
            # mg_split._grid_price_import =  mg_split._grid_price_import[starts[i]:stops[i]]
            # mg_split._grid_price_export =  mg_split._grid_price_export[starts[i]:stops[i]]
            # mg_split._grid_co2 =  mg_split._grid_co2[starts[i]:stops[i]]
            mg_split.horizon = 1000
            mg_split._data_length = 1000
            mg_split._next_grid_status = mg_split._next_grid_status[
                starts[i] : stops[i]
            ]
            # mg_split._next_grid_price_export =  mg_split._next_grid_price_export[starts[i]:stops[i]]
            # mg_split._next_grid_price_import =  mg_split._next_grid_price_import[starts[i]:stops[i]]
            # mg_split._next_grid_co2 =  mg_split._next_grid_co2[starts[i]:stops[i]]

            split_microgrid.append(mg_split)

        return split_microgrid
    ...
    def _create_microgrid(self):
        ...
        if architecture["grid"] == 1:
            ...
            grid_co2_ts = (
                self._get_co2_ts()
            )  # pd.read_csv(self.path+'/data/co2/co2_caiso.csv') #
